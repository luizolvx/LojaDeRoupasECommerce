...
public class DBQuery {
	...
	// ... (código existente do professor)

	// ADICIONE ESTE NOVO MÉTODO AO FINAL DA CLASSE DBQuery
	// ----------------------------------------------------------------------------------

	    // Este método é público e estático para ser chamado diretamente pelo Servlet
	    public static List<br.com.loja.modelo.Produto> buscarProdutosEmDestaque() {
	        // Importante: Note que o DBConnection é instanciado aqui!
	        DBQuery db = new DBQuery(); 
	        
	        List<br.com.loja.modelo.Produto> produtos = new ArrayList<>();
	        // Query específica para buscar produtos em destaque
	        // *Altere 'produto' se o nome da sua tabela for diferente*
	        String sql = "SELECT id, nome, preco, categoria FROM produto WHERE destaque = TRUE"; 

	        // O método query retorna um ResultSet
	        ResultSet rs = db.query(sql); 

	        if (rs != null) {
	            try {
	                // Itera sobre o resultado e preenche o objeto Produto
	                while (rs.next()) {
	                    br.com.loja.modelo.Produto p = new br.com.loja.modelo.Produto();
	                ...
	                System.err.println("Erro ao mapear ResultSet para Produto: " + e.getMessage());
	                e.printStackTrace();
	            } finally {
	                // Fecha a conexão (Importante! O DBQuery original não fecha sozinho a conexão)
	                try {
	                    if (rs != null) rs.close();
	                    if (db.statement != null) db.statement.close();
	                    // Como a conexão foi criada no construtor do DBQuery, ela precisa ser fechada
	                    // pelo objeto DBQuery (que contém o Statement).
	                } catch (SQLException e) {
	                    e.printStackTrace();
	                }
	            }
	...
	// ... (resto do código existente do professor)
